...
class DrivingClient(DrivingController):
    ...
    def control_driving(self, car_controls, sensing_info):
        ...
        # 속도에 따른 목표 웨이포인트 결정
        if speed < 120: target_waypoint = 5
        elif speed < 140: target_waypoint = 7
        elif speed < 182: target_waypoint = 8
        else: target_waypoint = 8; car_controls.throttle = 0.5
        ...
        if abs(angles[target_waypoint + 2]) < 49 or abs(angles[target_waypoint + 1]) < 47:
            if abs(angles[-1]) > 90 and speed > 165:
                if angles[-1] >= 0:
                    target_theta = atan((waypoints[target_waypoint][1] + 9) / waypoints[target_waypoint][0]) * to_degree - sensing_info.moving_angle
                else:
                    target_theta = atan((waypoints[target_waypoint][1] - 9) / waypoints[target_waypoint][0]) * to_degree - sensing_info.moving_angle
                car_controls.steering = target_theta / 180
            else:
                if speed < 120:
